...
class Student {
  ...
  _visits: Array<{ lesson: string; present: boolean }> = [];
  // _visits: Record<string, number> = {}; // lesson: present
  ...
  setVisits(lesson: string, present: boolean): void {
  ...
  getPerformanceRating(): number {
    ...
    const attendancePercentage = (this._visits.filter(visits => visits.present).length / this._visits.length) * 100;
